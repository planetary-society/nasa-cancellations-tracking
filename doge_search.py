...
class DOGEQuery(ContractQuery):
    ...
    def __init__(
        ...
        if "Award ID" not in final_columns:
            ...
            print(
                f"Warning: 'Award ID' added to final_columns for {self.__class__.__name__}",
                file=sys.stderr,
            )
    ...
    def _is_nasa_agency(self, item: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
        ...
        if isinstance(agency_value, str):
            ...
            if (
                lowercase_agency == self.TARGET_AGENCY_FULL_LOWER
                or lowercase_agency == self.TARGET_AGENCY_SHORT_LOWER
            ):
    ...
    def _extract_award_id_from_contract_url(self, url: Optional[str]) -> str:
        ...
        if isinstance(url, str) and url:
            try:
                ...
                self._log(
                    f"Warning: Could not parse Award ID (PIID) from a provided URL. Error: {e}"
                )
                # award_id remains ""
        return award_id
    ...
    def _extract_usa_spending_award_id_from_grant_url(self, url: Optional[str]) -> str:
        ...
        The URL's last path segment is USAspending's *generated* award id
        (e.g. ASST_NON_80NSSC24K0913_8000), not the FAIN. Returning it verbatim
        made every DOGE grant unmatchable against the award lookup - 26 claims
        dropped per run - and gave the same award two identities when another
        source reported it by FAIN.

        Args:
            url: The URL string in the form of https://usaspending.gov/award/ASST_NON_23K75IL000001_1605

        Returns:
            The extracted FAIN as a string, or an empty string if not found
            or if an error occurs during parsing.
        """
        award_id = ""
        if isinstance(url, str) and url:
            try:
                ...
                if path_parts:
                    ...
                    award_id = award_id_from_generated_id(award_id)
            except Exception as e:
                self._log(
                    f"Warning: Could not extract Award ID from usaspending URL. Error: {e}"
                )
        return award_id
    ...
    def _standardize_doge_item(
        ...
        if item_type == "Contract":
            ...
            standardized = {
                ...
                "description": "Status: "
                + item.get("fpds_status", "")
                + ". Reported savings: "
                + savings
                + ". DOGE Action Date: "
                + item.get("deleted_date")
                + ". "
            ...
            self._log(
                f"Warning: Unknown item_type '{item_type}' during DoGE standardization."
            )
            return None
    ...
    def _fetch_and_process_endpoint(
        ...
        while True:
            ...
            if total_pages:
                self._log(f"/{total_pages}", end="")
            self._log(" ", end="")

            try:
                response = requests.get(
                    endpoint_url, params=params, timeout=self.timeout
                )
                ...
                if not items_on_page:
                    self._log(
                        f"No more {item_type_name}s found. Fetch complete for this endpoint."
                    )
                    break
                ...
                for item in items_on_page:
                    ...
                    if is_match:
                        ...
                        standardized = self._standardize_doge_item(
                            item, item_type_name, agency_name_found
                        )
                        if standardized:
                ...
                if nasa_items_found_on_page > 0:
                    self._log(
                        f"  -> Found and processed {nasa_items_found_on_page} NASA {item_type_name}(s) on this page."
                    )
        ...
        self._log(
            f"--- Finished DoGE fetch for {item_type_name}s. Found {len(all_standardized_nasa_items)} NASA items. ---"
        )
    ...
    def search(self, **kwargs) -> pd.DataFrame:
        ...
        if kwargs:
            self._log(
                f"Note: search() called with keyword arguments {kwargs.keys()}, which are ignored by this NASA-specific implementation."
            )

        self._log(
            f"Initiating DoGE search specifically for NASA ({self.TARGET_AGENCY_DISPLAY_NAME})..."
        )
        ...
        grants_data = self._fetch_and_process_endpoint(
            endpoint_url=self.grants_endpoint, data_key="grants", item_type_name="Grant"
        )
        ...
        if combined_data:
            ...
            for col in self.final_columns:
                if col not in temp_df.columns:
                    self._log(
                        f"Note: Adding missing column '{col}' to the DataFrame (unexpected)."
                    )
                    temp_df[col] = pd.NA
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not nasa_data_df.empty:
            ...
            nasa_data_df.to_csv(
                filename, index=False, quoting=csv.QUOTE_ALL, escapechar="\\"
            )
