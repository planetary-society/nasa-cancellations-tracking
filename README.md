...
3.  **NASA Grants API (`nasa_grants_query.py`):** Queries NASA's Grant Search Form for workflow labels that suggest a period-of-performance decrease. The label does not identify the prior date or the transaction that changed it, so it is only a candidate: processing keeps it when `verification/award_period_change_facts.csv` contains a qualifying transaction-level mirror fact. Unconfirmed labels are excluded and retried after a later successful mirror refresh. If _no_ candidate can be confirmed and the mirror — the only producer of those facts — was itself skipped, this source is declared skipped rather than reporting zero rows: an unconfirmable source is unknown, not empty, and reporting zero would trip the presence and shrinkage guards and quarantine every run for as long as the mirror is away.
...
`consolidated/master_ledger.csv` is the artifact to cite, so its column names are meant to be readable without this file. Where a name could still mislead:

| Column                                 | What it actually holds                                                                                                                                                                                                                                                                                                                                                                           |
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `Current Obligated Amount`             | USAspending's `total_obligation` — money the government is **obligated** to pay. Deliberately _not_ "Current Award Amount", which is USAspending's own glossary term for base-plus-exercised-options, a contract ceiling this tracker never fetches.                                                                                                                                             |
| `Obligated Amount When First Flagged`  | The same figure as first observed by this tracker, which is usually long after the award began.                                                                                                                                                                                                                                                                                                  |
| `Peak Cumulative Obligation`           | The highest the running obligation total ever reached across the award's full transaction history. Populated only where the first observation was a zero-dollar action; blank means "not needed", not "unknown".                                                                                                                                                                                 |
| `Current End Date`                     | `period_of_performance_current_end_date`, whose own API help text reads "The current, not original, period of performance end date". USAspending publishes none for an IDV vehicle, so those rows carry `Last Date to Order` instead — when orders may no longer be placed, not when work ends. **Check `End Date Basis` before aggregating over this column**; it holds two different measures. |
| `End Date Basis`                       | Which measure the row's `Current End Date` is: `period_of_performance` or `idv_last_date_to_order`. Blank on rows whose most recent snapshot predates the column (added 2026-08); it is not back-inferred from the award type, because an IDV that does publish a period end uses that instead.                                                                                                  |
| `Initial Reported End Date`            | The end date on the award's base transaction, or the earliest transaction carrying one. "Reported", not "originally awarded", because the fallback is a modification.                                                                                                                                                                                                                            |
| `End Date When First Flagged`          | Tracker-era, not award-era: what the end date was the first day we saw the award.                                                                                                                                                                                                                                                                                                                |
| `Award or Action Description`          | Usually the terminating transaction's text, not the award's own blurb — the flagging source's description wins, and the award description is the fallback.                                                                                                                                                                                                                                       |
| `Detection Evidence`                   | Why _this tracker_ flagged the award, in the flagging source's words. `Primary Detection Method` is its structured counterpart, and keeps "Primary" because it names only the winner when several nets fire.                                                                                                                                                                                     |
| `Flagged By`                           | Which of our detectors found it, accumulated over time. Distinct from `Claimed By`, which is who _asserted_ a cancellation.                                                                                                                                                                                                                                                                      |
| `Tracking Status`                      | This tracker's classification of the award, not the award's own status. `currently_flagged` means a source still returns it today.                                                                                                                                                                                                                                                               |
| `Automated Verdict Date`               | When the machine verdict last **changed**, not when the award was last checked.                                                                                                                                                                                                                                                                                                                  |
| `Action Code Termination Modification` | The modification carrying an official FPDS termination action code. Citable straight against the source, and independent of any keyword list — but blank on most terminated awards, because NASA routinely records the termination under a generic code.                                                                                                                                         |
| `Verified Termination Modification`    | The modification this tracker concluded terminated the award, accepting a generic action code whose description asserts a termination and skipping rescissions. This is the anchor behind `Automated Verdict`. Blank until re-verification has reached the award.                                                                                                                                |
| `Total Outlays`                        | Account-level (File C), while obligations are transaction-derived (File D). Differencing the two is meaningless for awards with no File C linkage.                                                                                                                                                                                                                                               |
| `Recipient Congressional District`     | The **recipient's** district, not the place of performance — USAspending publishes both and they differ. Always a `SS-DD` pair or blank: foreign recipients have no district, and USAspending occasionally supplies a district number with no state, which is not citable and is dropped rather than published half-formed.                                                                      |

Columns renamed in 2026-08 are translated on read, so the 400-odd archived daily snapshots — the record of what was published on each day — are never rewritten. The old-to-new map is `csv_aliases.py`. That translation is internal to this repo: the current `master_ledger.csv` carries only the new names, so anything downstream that reads the published file by column name needs updating once.

## Claims vs. Outcomes

The tracker records two different kinds of fact, and the ledger keeps them apart.

A **claim** is an external assertion that an award was cancelled — currently only DOGE makes them. The claim is the thing being tracked, so it is retained permanently and is never pruned, whether or not the award turned out to be genuinely terminated. Claims are captured in `Claimed By`, `DOGE Claimed Status`, `DOGE Claimed Savings`, and `DOGE Claim Date` — prefixed because they are the claimant's assertions rather than anything this tracker observed, and `DOGE Claimed Savings` in particular is a number a reader could otherwise mistake for a finding of ours. These are **write-once**: a later snapshot in which a different source flags the award cannot erase them. When a source restates a claim, the change is appended to `DOGE Claim Revisions` rather than overwriting the original assertion.

An **outcome** is what the award data actually shows happened since: `Amount Trend`, `End Date Trend`, and `DOGE Claim vs Outcome` (`claimed_but_grew`, `claimed_but_extended`, `claimed_and_shrank`, `consistent`). These are derived on every build from the first and latest values. When an award was first observed on a zero-dollar action, `Peak Cumulative Obligation` supplies the maximum cumulative obligation from its complete USAspending transaction history without overwriting `Obligated Amount When First Flagged`; an incomplete or nonnumeric history is marked `unknown`. A computed zero also leaves the trend `unknown` because percentage change from zero is undefined. `Initial Reported End Date` comes from the base transaction in the award's full history, or the earliest nonblank transaction when the base row has no date; `End Date Trend` prefers it and falls back to the tracker-era `End Date When First Flagged`.

Both trend columns are deliberately unqualified. Naming them "since first flagged" would be wrong on the awards that compute from `Peak Cumulative Obligation`, which predates flagging — a short vague name beats a long false one. It is resolved **only from the local mirror** — see [Initial Reported End Date](#initial-reported-end-date) below for why, and for what happens without mirror access. For IDV vehicles, both the initial and current dates fall back to USAspending's `Last Date to Order` when the generic period end is absent — so the two ends of the comparison are the same measure, and a `truncated` IDV means its ordering window was cut, which for a blanket purchase agreement is the cancellation event. `End Date Basis` marks those rows. `claimed_but_grew` is the notable case — an award whose claimed savings is contradicted by the award subsequently growing.

Daily enrichment fetches each award's complete USAspending transaction history once and reuses it for every transaction-derived field. `First Action Type` and `Latest Action Type` (plus their descriptions and action dates) report the endpoints of that history. The award's latest modification is therefore reported as the pair `Latest Modification Number` and `Latest Action Date`, both read from that final transaction. An earlier `Latest Modification Date` column was removed: it was sourced from `award.period_of_performance.last_modified_date`, which USAspending defines as when the award _record_ was last updated rather than when its latest transaction occurred, and the two routinely diverged by months. `Action Code Termination Modification` and `Closeout Modification Number` retain the most recent formal action of each kind, together with its action date, even when a later continuation or administrative modification becomes the award's latest transaction. Contract and assistance action codes are interpreted separately because the same code can mean different things in the two systems.

The ledger publishes **two** answers to "which modification terminated this award", because they disagree on roughly a quarter of tracked awards and each is right about something different. `Action Code Termination Modification` is the formal FPDS record; `Verified Termination Modification` is what weekly re-verification concluded, which additionally accepts a generic action code whose description asserts a termination and steps over rescissions so a reversed modification cannot become the anchor. Neither is a superset of the other: a termination for cause carries a real action code but is excluded from the verified answer by methodology, while a stop-work order recorded under "other administrative action" appears only in the verified one. Both rules live in `award_transaction_facts.terminating_index`, which takes the policy as an argument — they were previously two separate scans in two modules and drifted apart unnoticed.
...
`reverify_awards.py` walks each award's transactions and decides whether a termination stands, was superseded by a closeout, or was reversed. The same complete history supplies `Peak Cumulative Obligation` for zero-dollar first observations. It runs weekly via `.github/workflows/reverify.yml`.
...
Only high-confidence verdicts can set a ledger `Tracking Status`; verdicts resting on the _absence_ of evidence never can. The `Disagrees With Human` column surfaces conflicts for review without ever applying them.

```bash
python reverify_awards.py --dry-run                    # show selection, make no calls
python reverify_awards.py --max-awards 30              # bounded run
python reverify_awards.py --award-id 80HQTR22F0076
python reverify_awards.py --recheck-after-days 60      # skip anything checked since
python reverify_awards.py --include-excluded-by-design # also re-check human exclusions
python reverify_awards.py --skip-ledger-rebuild        # leave the ledger alone
```

The flags were renamed in 2026-08 (`--max-requests`, `--stale-days`, `--include-excluded`, `--no-rebuild`); the old spellings still work. `--recheck-after-days` is a base rather than a flat threshold — the selection tiers scale it, so the cheapest tier waits three times as long and the most valuable ignores it.

Rebuilding the ledger:

```bash
python build_master_ledger.py                       # full replay of every snapshot
python build_master_ledger.py --latest-snapshot-only # incremental; was --update
```

Use a full rebuild for anything that will be published: the incremental path only reads the newest snapshot, so it cannot infer the statuses that depend on older description text.
