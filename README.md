...
| `Current End Date`                     | `period_of_performance_current_end_date`, whose own API help text reads "The current, not original, period of performance end date". For the IDV vehicles it falls back to `Last Date to Order`, which is when orders may be placed, not when work ends.                                                                    |
...
Both trend columns are deliberately unqualified. Naming them "since first flagged" would be wrong on the awards that compute from `Peak Cumulative Obligation`, which predates flagging — a short vague name beats a long false one. It is resolved **only from the local mirror** — see [Initial Reported End Date](#initial-reported-end-date) below for why, and for what happens without mirror access. For IDV vehicles, both the initial and current dates prefer USAspending's `Last Date to Order` when the generic period end is absent. `claimed_but_grew` is the notable case — an award whose claimed savings is contradicted by the award subsequently growing.
