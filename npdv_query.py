...
class NPDVQuery(ContractQuery):
    ...
    *only in the latest modification* for each unique Award ID.
    ...
    # Deliberately broader than termination_vocabulary.TERM_TEXT: this is the
    # detection net over every NASA contract mod, where a bare "termination" is
    # worth surfacing for review. The narrow shared pattern is for classifying
    # text already known to concern a tracked award.
    ...
    DEFAULT_CSV_URL_2026 = "https://raw.githubusercontent.com/planetary-society/nasa-contracts/refs/heads/master/data/nasa_awards_2026.csv"
    DEFAULT_CSV_URLS = [
        DEFAULT_CSV_URL_2025,
        DEFAULT_CSV_URL_2026,
    ]  # Order matters: FY2025 first, then FY2026
    ...
    def __init__(
        ...
        self.search_phrases = (
            ...
            else self.DEFAULT_SEARCH_PHRASES
        )
        ...
        for url in self.csv_urls:
            ...
            if filename:
                self._local_filepaths.append(
                    (url, os.path.join(self.local_cache_dir, filename))
                )

        if self.search_phrases:
            self._search_pattern_re = re.compile(
                r"\b(?:"
                + "|".join(re.escape(phrase) for phrase in self.search_phrases)
                + r")\b",
                re.IGNORECASE,
            )
        else:
            logging.warning(
                "No search phrases provided; search will likely return no results."
            )
    ...
    def _format_date_iso(date_str: str) -> str:
        ...
        try:
            ...
            logging.warning(
                f"Could not parse date string '{date_str}' using format '{input_format}'. Returning empty string. Error: {e}"
            )
        ...
        try:
            ...
            if os.path.exists(filepath):
                try:
                    ...
                    logging.error(
                        f"Failed to remove incomplete download {filepath}: {rm_err}"
                    )
            return False
        except OSError as e:
            logging.error(f"Failed to write downloaded file to {filepath}: {e}")
            return False
        except Exception as e:
            logging.error(f"An unexpected error during download: {e}", exc_info=True)
            return False
    ...
    def _get_data_filepath(
        ...
        if force_reload:
            logging.info(
                f"Force reload requested. Downloading fresh data to {filepath}..."
            )
    ...
    def search(self, force_reload: bool = False, **kwargs) -> pd.DataFrame:
        ...
        if kwargs:
            logging.warning(
                f"{self.__class__.__name__}.search received unused parameters {kwargs.keys()}."
            )
        ...
        for url, filepath in self._local_filepaths:
            local_filepath = self._get_data_filepath(
                url, filepath, force_reload=force_reload
            )
            ...
            logging.info(
                f"Scanning CSV file to find latest modifications: {local_filepath}"
            )
            try:
                with open(
                    local_filepath, mode="r", newline="", encoding="utf-8"
                ) as csvfile:
                    ...
                    if not reader.fieldnames:
                        logging.error(
                            f"CSV file '{local_filepath}' appears to be empty or header is missing."
                        )
                        continue
                    if not all(col in reader.fieldnames for col in required_cols):
                        ...
                        logging.error(
                            f"CSV file is missing required columns: {missing}. Skipping."
                        )
                        continue
                    ...
                    for row in reader:
                        ...
                        try:
                            ...
                            logging.error(
                                f"Error processing row {row_count} during scan: {row}. Error: {e}",
                                exc_info=False,
                            )

                    logging.info(
                        f"Finished scanning {row_count} rows from {local_filepath}."
                    )
                ...
                logging.error(
                    f"An unexpected error occurred during CSV scanning: {e}",
                    exc_info=True,
                )
                continue
        ...
        logging.info(
            f"Found {len(latest_rows)} unique Award IDs with latest modifications across all files."
        )
        ...
        for award_id, (mod_num, row_dict) in latest_rows.items():
            try:
                ...
                if description and self._search_pattern_re.search(description):
                    ...
                    logging.debug(
                        f"Award ID {award_id}: Original Completion Date='{completion_date_str}', Formatted='{formatted_deleted_date}'"
                    )
                    # --- End Formatting ---
                    ...
                    filtered_output_data = {
                        k: v
                        for k, v in output_row_data.items()
                        if k in self.final_columns
                    }
                    final_results_list.append(filtered_output_data)

            except Exception as e:
                logging.error(
                    f"Error filtering or formatting latest mod for Award ID {award_id}: {e}",
                    exc_info=False,
                )
        ...
        if not final_results_list:
            logging.info(
                "No records found where the latest modification matched the termination criteria."
            )
            return pd.DataFrame(columns=self.final_columns)
        else:
            try:
                ...
                logging.info(
                    f"Created final DataFrame with {len(final_df)} terminated contracts based on latest modification."
                )
                ...
                logging.error(
                    f"Failed to create final DataFrame from results: {e}", exc_info=True
                )
                return pd.DataFrame(columns=self.final_columns)
...
if __name__ == "__main__":
    print(
        "--- Running NPDVQuery Example (Filtering by Latest Mod using csv.DictReader) ---"
    )
